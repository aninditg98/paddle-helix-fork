...
def make_sequence_features(sequence: str, description: str, num_res: int) -> FeatureDict:
    ...
    features['aatype'] = residue_constants.sequence_to_onehot(
        sequence=sequence, mapping=residue_constants.restype_order_with_x, map_unknown_to_x=True
    )
...
def run_msa_tool(
    ...
    print("Checking if output already exists first:", msa_out_path)
...
class DataPipeline:
    ...
    jackhmmer_mgnify_result = run_msa_tool(
        ...
        with ProcessPoolExecutor() as executor:
            futures = {
                executor.submit(run_msa_tool_wrapper, msa_task): msa_task for msa_task in msa_tasks
            }
        ...
        # # pdb_templates_result = open(pdb_hits_out_path, 'r').read()
        ...
        logging.info(
            'Final (deduplicated) MSA size: %d sequences.', msa_features['num_alignments'][0]
        )
