...
def _extract_template_features(
  ...
  Args:
    ...
    template_chain_id: String ID describing which chain in the structure proto
      should be used.
    confidence_scores (deprecated): String containing per-residue confidence scores,
      where each character represents the *TRUNCATED* posterior probability that the
  ...
  if np.sum(templates_all_atom_masks) < 5:
    mv = list(mapping.values())
    if len(mv) == 0:
      mv = [0]
    raise TemplateAtomMaskAllZerosError(
        'Template all atom mask was all zeros: %s_%s. Residue range: %d-%d' %
        (pdb_id, chain_id, min(mv) + mapping_offset, max(mv) + mapping_offset))
